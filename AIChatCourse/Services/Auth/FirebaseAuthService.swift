...
extension EnvironmentValues {
    @Entry var authService: FirebaseAuthServices = FirebaseAuthServices()
}
        ...
        if let user = Auth.auth().currentUser, user.isAnonymous {
            do {
                ...
                case .providerAlreadyLinked, .credentialAlreadyInUse:
                    if let secondaryCredetial = error.userInfo["FIRAuthErrorUserInfoUpdatedCredentialKey"] as? AuthCredential {
                        let result = try await Auth.auth().signIn(with: secondaryCredetial)
                        return result.asAuthInfo
                    }
                    break
                default:
